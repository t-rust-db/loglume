...
### Changed
- db-core pinned to v0.114.0 (was v0.101.6, thirteen minors behind). No loglume code changes were needed; `make test`, `make lint` and `make smoke` pass unchanged.
...
### Added
- Press `f` to filter down to only the currently highlighted lines, toggling back to the prior filter state on a second press (#70). The highlight expression is promoted into the pane's active SQL filter without clearing the highlight itself — marking still applies on top of the narrowed results.
...
### Added
- Press `b` to collapse the filter and highlight bars, freeing the log list to use the full pane height — useful when watching a live-tailed log (#58). Bars are automatically shown when entering filter (`/`) or highlight (`?`) edit mode.

### Changed
- The list's border title shows `[bars hidden]` when the bars are collapsed, so the state remains visible without the bars themselves.
